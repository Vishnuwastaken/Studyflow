...
export function renderHomePage(ctx){
  const { recentDeck, state, petMood, petColor, xpForLevel } = ctx;
  const d = recentDeck();
  const xp = xpForLevel(state.pet.level);
  return `
    <div class="card"><div class="space"><div><div class="h1">Welcome back</div><div class="small">Ready to keep your streak going?</div></div>${renderPetDisplay({color:petColor(), hat:!!state.pet.equipped.hat, glasses:!!state.pet.equipped.face})}</div></div>
    ...
    <div class="grid2"><div class="card"><div class="small">Daily progress</div><div class="h1" style="margin-top:6px">${state.user.daily.studied} cards</div><div style="height:10px"></div>${renderProgressBar(Math.min(100, state.user.daily.studied * 10))}</div><div class="card"><div class="small">Current streak</div><div class="h1" style="margin-top:6px">${state.user.streak.current} day${state.user.streak.current===1?'':'s'}</div></div></div>
