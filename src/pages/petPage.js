...
export function renderPetPage(ctx){
  const { state, items, xpForLevel, petMood, petColor, shopError, feedCost } = ctx;
  ...
  const itemRow = i => {
    const equipped = state.pet.equipped[i.category] === i.id;
    return `<div class="panel"><div class="space"><div class="row"><div class="icon">${i.icon}</div><div><div class="h3">${esc(i.name)}</div><div class="small">${esc(i.category)}</div></div></div>${equipped?`<button class="btn warning inline" onclick="unequipItem('${i.category}')">Unequip</button>`:`<button class="btn inline" onclick="equipItem('${i.id}')">Equip</button>`}</div></div>`;
  };
