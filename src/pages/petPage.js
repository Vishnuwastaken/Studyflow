...
export function renderPetPage(ctx){
  ...
  const stageLabels = ['Baby','Growing','Formed','Mature','Evolved'];
  const stageIndex = Math.max(1, Math.min(5, petEvolutionStage()));
  ...
  return `<div class="card"><div style="text-align:center"><div class="h1">${esc(state.pet.name)}</div><div class="small">Level ${state.pet.level} · Stage ${petEvolutionStage()}/5 (${stageLabels[stageIndex - 1]}) · ${petMood()} mood</div><div class="h3" style="margin-top:8px">${state.user.points} points</div><div style="display:flex;justify-content:center;margin:14px 0">${renderPetDisplay({big:true,color:petColor(),hat:!!state.pet.equipped.hat,glasses:!!state.pet.equipped.accessory,mood:state.pet.mood,stage:petEvolutionStage(),celebrate:(Date.now() - (state.pet.lastLevelUpAt || 0)) < 4000})}</div>${renderProgressBar(Math.round(state.pet.xp / xp * 100))}<div class="tiny" style="margin-top:8px">${state.pet.xp} / ${xp} XP to next level</div><div class="tiny" style="margin-top:6px">Feeding gives a small XP bonus and mood boost</div><div style="height:12px"></div><button class="btn inline" onclick="feedPet()">Feed Pet (${feedCost} points)</button></div></div><div class="card"><div class="h2">Inventory</div><div style="height:10px"></div>${owned.length?`<div class="list">${owned.map(itemRow).join('')}</div>`:'<div class="empty">Buy an item from the shop below.</div>'}</div><div class="card"><div class="h2">Shop</div>${shopError?`<div class="tiny" style="margin-top:8px;color:var(--danger)">${esc(shopError)}</div>`:''}<div style="height:10px"></div>${shop.length?`<div class="list">${shop.map(shopRow).join('')}</div>`:'<div class="empty">You own everything in the shop.</div>'}</div>`;
}
