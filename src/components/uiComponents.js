...
export function renderPetDisplay({ big=false, color='#8cc8ff', hat=false, glasses=false, mood='calm', stage=1, celebrate=false }){
  ...
  return `<div class="pet-wrap ${big ? 'big' : ''}"><div class="pet-shadow"></div><div class="pet ${big ? 'big' : ''} ${evolutionClass} ${moodClass} ${celebrate ? 'level-up' : ''}" style="--pet-color:${color}"><div class="ear left"></div><div class="ear right"></div><div class="pet-body"></div><div class="pet-mark"></div>${hat?'<div class="hat"></div>':''}${glasses?'<div class="glasses"><i></i></div>':''}<div class="eye left"></div><div class="eye right"></div><div class="blush left"></div><div class="blush right"></div><div class="mouth ${mood}"></div></div></div>`;
}
